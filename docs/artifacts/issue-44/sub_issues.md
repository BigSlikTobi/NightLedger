...
## Sub-issue 4 (active in this branch)

- Scope: Add a real MCP stdio server wrapper that exposes `authorize_action`
  as an MCP tool callable by agent clients.
- Boundaries:
  - Reuses existing authorize-action transport contract logic.
  - Does not add policy threshold governance behavior from issue #45.
  - Does not add executor token enforcement from issue #47.
- Done when:
  - MCP server responds to initialize, tools/list, and tools/call.
  - `tools/call` invokes authorize-action and returns contract-compliant
    decision payloads.
  - Invalid arguments return structured validation errors through MCP tool-call
    error results.
